...
class ItemConfigTimeGroup extends GetView<GroupController> {
  const ItemConfigTimeGroup(
  ...
  final CategoryModel? application;
  ...
  Widget _buildItem(int index, RuleAppModel element) {
    return ItemTimeInfoGroup(
        timeModel: element, onDelete: () => onDelete?.call(element), application: application);
  }
}

class ItemTimeInfoGroup extends GetView<GroupController> {
  const ItemTimeInfoGroup({this.application, Key? key, this.timeModel, this.onDelete}) : super(key: key);
...
class ItemDayInfo extends StatelessWidget {
  const ItemDayInfo({Key? key, this.title, this.isActive, this.onActive}) : super(key: key);
  final String? title;
  final bool? isActive;
