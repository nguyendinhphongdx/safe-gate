...
class LastWeekController extends IReportListener {
  CountTimer countTimer = CountTimer(500);
  Rx<ReportProfileModel?> mReportProfileModel = ReportProfileModel().obs;

  @override
  Future initialData() async {       mProfiles = Get.find<ReportController>().mProfiles;
  await fetchStatuses();
  ...
  Future<void> fetchReport() async {
    ApiResponse<ReportByDayModel?> response = await GetReportByProfileIdUseCase(
            ...
            dateTimes: getListDateFilter(7, DateTime.now().subtract(const Duration(days: 7))))
        .invoke();
  ...
  Future<void> _callDataByUserSelected() async {
    ...
    profileSelected.value = selectedTmp;    await fetchStatuses();
    fetchHistory();
  ...
  Future<void> fetchHistory() async {
    ...
    mHistories.value = List.generate(
        ...
        (index) => ViolationHeaderData(
            dateTime: DateTimeUtils.format(now.subtract(Duration(days: 7 + index)), DateTimeUtils.YYYYMMDD),
            id: index,
            date: now.subtract(Duration(days: 7 + index)),
            expandableController: (index == 0) ? (ExpandableController()..toggle()) : null,
            title: DateTimeUtils.getTitleByIndex(index + 7, now)));
  }

  @override
  Map<String, String> getParamForHistory() {
    return TimeLineHistoryArgs(
      startTime: DateTimeUtils.format(DateTime.now().subtract(const Duration(days: 14)), DateTimeUtils.DDMMYYYY),
      endTime: DateTimeUtils.format(DateTime.now().subtract(const Duration(days: 7)), DateTimeUtils.DDMMYYYY),
      profileId: profileSelected.value?.profileId.toString(),
    ).toJson();
  }
